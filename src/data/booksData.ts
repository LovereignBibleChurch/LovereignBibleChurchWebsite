const bookImage1 = "/books/giftsandcalling.jpg"
const bookImage2 = "/books/systems_structures.jpg"
const bookImage3 = "/books/work_of_ministry.jpg"
const bookImage4 = "/books/categoriesoffellowers.png"
const bookImage5 = "/books/excuses.png"
const bookImage6 = "/books/honor.png"
const bookImage7 = "/books/howwegotourbible.png"
const bookImage8 = "/books/offences.png"
const bookImage9 = "/books/salvation.png"

export const BooksData = [
    {
        id: 1,
        title: "The Gifts and Calling of God",
        slug: "gifts-and-calling",
        image: bookImage1,
        description:
            "An insightful exploration of spiritual rebirth and the transformative journey of faith. This book delves into the profound meaning of being 'born again' and how it changes one's perspective on life.",
        price: "$20",
        pages: "214",
        year: "2025",
        category: "Spiritual Growth",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "Chapter 1: Gifts and Calling",
            content: [
                "God’s gifts and calling are irrevocable, yet many struggle to fully walk in them. Why do some ministers thrive while others falter? What separates a fruitful ministry from one that fades into obscurity? Ministry is deeply spiritual; without the supernatural element, it becomes a mere social gathering. The manifestation of gifts sustains true service, marking the beginning of a believer’s walk with God. The author establishes that every biblical calling was ultimately a call to build a nation, the church, the city of God, and His tabernacle among men.",
                "In The Gifts and Calling of God, you will discover the many gifts given to believers, all working toward a singular purpose to build the church, the nation of God. This book explores the principles, characteristics, and signs that mark the receiving and manifestation of spiritual gifts, emphasizing their divine nature in establishing God’s nation, the church, the ground, and the pillar of truth.",
                "Through biblical insights and practical wisdom, you will gain understanding of:",
                
                "1. How to identify and activate your God-given gifts.",
                "2. The role of gifts in ministry growth and impact.",
                "3. How to follow divine pathways and receive from those God sends your way.",
                "4. The three major categories of gifts are those that serve in support, manifest divine power, and establish leadership, authority, and spiritual governance.",
                "Whether you are a seasoned minister, just beginning your journey, or simply seeking relevance in God’s house, this book will equip you with the knowledge, wisdom, and spiritual understanding needed to walk in your calling and divine assignment with confidence and impact.",
                "Are you ready to embrace your calling and fulfill your ministry? The Gifts and Calling of God will show you the way.",
            ],
        },
    },
    {
        id: 2,
        title: "Church Systems and Structures For Growth",
        slug: "church-systems-and-structures-for-growth",
        image: bookImage2,
        description:
            "Church Systems and Structures for Growth equips church leaders with 13 essential systems to strengthen governance, develop leaders, and drive sustainable ministry growth. Rooted in biblical principles and real ministry experience, it offers practical tools for building a thriving church. Ideal for pastors, administrators, and anyone preparing for ministry.",
        price: "$20",
        pages: "347",
        year: "2025",
        category: "Church Leadership",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book", "Audiobook"],
        sampleChapter: {
            title: "Chapter 1: What are Systems and Structures?",
            content: [
                
                "Every thriving ministry is built upon well-designed systems and structures. Just as the human body functions through interconnected systems, the body of Christ requires organizational frameworks that facilitate growth, health, and effectiveness.",
                
                "In 1 Corinthians 14:40, Paul instructs that 'all things should be done decently and in order.' This biblical principle underscores the importance of intentional organization within the church. Without proper systems, even the most passionate ministry efforts can become chaotic and ultimately ineffective.",
                
                "Why Systems Matter",
                
                "Systems provide the necessary infrastructure for sustainable growth. They allow churches to:",
                
                "• Efficiently steward resources",
               
                "• Effectively disciple new believers",

                "• Consistently deliver ministry services",

                "• Seamlessly integrate new members",

                "• Strategically expand outreach efforts",
               
                "When Jesus fed the five thousand, He implemented a system—having people sit in groups, distributing the food through the disciples, and collecting the leftovers. This demonstrates that even miraculous ministry benefits from systematic approaches.",
                
                "In the following chapters, we will examine 13 essential systems and structures that empower churches to flourish.",
            ],
        },
    },
    {
        id: 3,
        title: "The Work of Ministry",
        slug: "the-work-of-ministry",
        image: bookImage3,
        description:
            "A comprehensive guide to understanding the various aspects of ministry work and how to effectively serve in your calling. This book offers wisdom for both new and experienced ministers.",
        price: "$20",
        pages: "576",
        year: "2025",
        category: "Ministry & Calling",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "Chapter 1: The Work of the Ministry",
            content: [
                // 1
                "The Sacred Call to Ministry",
                // 2
                "Ministry is a sacred calling, but it is also a battlefield, one that demands divine direction, wisdom, endurance, and unwavering commitment. Many begin with zeal, yet only those who grasp the principles, understand spiritual warfare, and receive continuous divine strategy, endure to the end.",
                // 3
                "This book explores essential aspects of ministry, including practical church work, leadership succession, the different phases of ministry, critical pitfalls to avoid, the mindset required for effective ministry, the role and responsibilities of an assistant, strategies for overcoming setbacks, preservation of ministry focus, and many other vital insights.",
                // 4
                "Whatever phase of the ministry you are in, this book will challenge, inspire, and empower you to rise above obstacles, walk in the fullness of your ministry, and fulfill the call of God on your life.",
                // 5
                "You will learn how to:",
                // 6
                "• Approach ministry as a divine calling, not just a profession.",
                // 7
                "• Navigate spiritual warfare and overcome setbacks.",
                // 8
                "• Develop the right mindset and strategies for effective ministry.",
                // 9
                "• Understand the importance of leadership succession and the role of assistants.",
                // 10
                "• Preserve your focus and passion for the work of God.",
                // 11
                "Prepare to be transformed for the work of ministry!",
            ],
        },
    },
    {
        id: 4,
        title: "Categories of Followers in the Church",
        slug: "categories-of-followers-in-the-church",
        image: bookImage4,
        description:
            "Every ministry attracts people, but not all followers are the same. This book explores the various categories of followers in Jesus’ ministry, highlighting their distinct traits, behavioral patterns, and spiritual dispositions relevant for today’s church.",
        price: "$20",
        pages: "250",
        year: "2025",
        category: "Leadership & Ministry",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "Introduction: Understanding Followers",
            content: [
                "Every ministry attracts people, but not all followers are the same. In the days of Jesus, the crowds that followed His ministry were vast, yet Scripture shows that those who came to Him had different motives, attitudes, and levels of commitment.",
                "This book explores the various categories of followers in Jesus’ ministry, highlighting their distinct traits, behavioral patterns, and spiritual dispositions. Understanding these distinctions is essential for every pastor, leader, or servant of God.",
                "As you read, you will discover that these categories are not limited to biblical times but remain relevant in today’s church and in every organization. Whether you are leading a congregation, a ministry team, or a para-church organization, this book will equip you to:",
                "• Identify patterns among followers and sharpen your leadership",
                "• Recognize the types of people drawn to your leadership and ministry",
                "• Understand their strengths, weaknesses, and tendencies",
                "• Transform how you engage those entrusted to your care",
                "• Evaluate your own role as a follower in the body of Christ",
                "With practical insights drawn from the ministry of Jesus, this book is more than a leadership manual; it is a mirror for self-examination and a guide for effective shepherding in ministry and every leadership context.",
            ],
        },
    },
    {
        id: 5,
        title: "Excuses",
        slug: "excuses",
        image: bookImage5,
        description:
            "Excuses do not exist, people make them up. This book uncovers the hidden cost of excuses, how they limit vision, weaken discipline, and delay success, challenging leaders to replace them with accountability and action.",
        price: "$20",
        pages: "180",
        year: "2025",
        category: "Self-Help & Leadership",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "The Reality of Excuses",
            content: [
                "Excuses do not exist, people make them up. On the surface, excuses may seem to be opportunities to escape from being responsible, but beneath is a spiritual reality that they are subtle barriers people build to shield themselves from being held accountable and slow down the progress of any group.",
                "The enemy uses excuses to delay and destroy vision and potential success. The excuses people give may sound harmless, even seem reasonable, yet they quietly rob people, churches, and organizations from making advancements and denying them of potential achievements.",
                "This book uncovers the hidden cost of excuses, how they limit vision, how they weaken discipline, and how they delay success. With practical insight and clear examples, it challenges leaders to recognize the types and patterns of excuses and to replace them with accountability, courage, and with action that leads to progress.",
                "This book exposes the lies people tell themselves, the justifications they cling to, and it reveals that fulfillment and progress begins where excuses end.",
            ],
        },
    },
    {
        id: 6,
        title: "Honour",
        slug: "honor",
        image: bookImage6,
        description:
            "A church cannot grow beyond the honour practiced within it. This book explains why honour is a divine ordinance and eternal law, key to building and sustaining meaningful relationships within the body of Christ.",
        price: "$20",
        pages: "220",
        year: "2025",
        category: "Kingdom Principles",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "The Divine Ordinance of Honour",
            content: [
                "A church cannot grow beyond the honour practiced within it, especially the honour shown to its leaders. In Job 38:33, God asks Job, 'Do you know the laws of the heavens? Can you set their dominion over the earth?' This question reminds us that God’s kingdom operates by laws that function both in heaven and on earth.",
                "These laws include seasons, seedtime and harvest, faith, love, and others, each revealing God’s order and sovereignty. They serve as divine principles guiding human behaviour and relationships, showing that God’s kingdom functions by consistent laws across both realms.",
                "Among these laws is the law of honour. In heaven, honour governs interactions among heavenly beings; on earth, it determines the strength and longevity of human relationships. Lasting relationships and fruitful fellowship require understanding and practicing honour. The breakdown of relationships is ultimately a violation of this law.",
                "This book explains why honour is a divine ordinance and eternal law, why it is a fundamental human need, and how it is key to building and sustaining meaningful relationships. With practical guidance, it outlines principles for honouring others, especially pastors and ministers within the church.",
                "This book serves as both a treasure and a teaching manual, equipping readers to instruct congregations, strengthen relationships, and uphold kingdom principles among God’s people.",
            ],
        },
    },
    {
        id: 7,
        title: "How We Got Our Bible",
        slug: "how-we-got-our-bible",
        image: bookImage7,
        description:
            "The Bible is the world's most influential book. This book explores its divine origin, historical preservation, and the process of its formation, offering a clear window into the spirituality that shaped the Christian canon.",
        price: "$20",
        pages: "200",
        year: "2025",
        category: "Biblical Studies",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "The Formation of the Word",
            content: [
                "The Bible is the world’s most influential book, often called 'the Book' because it stands in a class of its own. The word Bible means 'books,' which indicates its nature as a collection of 66 writings authored by about 40 individuals across multiple regions over roughly 1,600 years. Yet, despite its variety of contributors, they all present a unified message without contradiction and this clearly points to an unmistakable divine origin.",
                "This book explores how through the Bible, God’s voice communicates to humanity. Contrary to claims that the Bible is a fabricated myth, its depth of wisdom, prophetic accuracy, moral authority, and enduring truth surpass anything human intellect alone could produce.",
                "Its authenticity is confirmed through scientific, historical, moral, and prophetic evidence, having withstood unparalleled academic scrutiny throughout history.",
                "Just as holy men spoke under the inspiration of the Holy Spirit, early Church Fathers carefully discerned, preserved, and compiled the writings that formed the Christian canon. This book traces that process, explaining the criteria used to recognize inspired texts and why others were excluded. Through historical insight and spiritual reflection, it reveals how the Bible was faithfully formed and preserved and offers a clear window into the devotion and deep spirituality that shaped the Bible as we know it today.",
            ],
        },
    },
    {
        id: 8,
        title: "Offenses",
        slug: "offenses",
        image: bookImage8,
        description:
            "Offense is inevitable, but how we respond determines the fate of our relationships. This book explores the anatomy of offense, its destructive power, and provides a guide to healing, forgiveness, and reconciliation.",
        price: "$20",
        pages: "190",
        year: "2025",
        category: "Relationships & Healing",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "The Anatomy of Offense",
            content: [
                "'Offenses must come,' said Jesus. It means Offense is inevitable and it unveils the true state of the human nature, that it is incapable of offending. But how we respond to offenses determine whether we destroy or redeem our relationships.",
                "Offense is more than a fleeting wound in the hearts of people, it is also a force that can transform ordinary people into dangerous beings, capable of inflicting unimaginable harm upon others. If left unchecked, offenses corrode relationships, distorts perception, and breeds cycles of bitterness.",
                "This book is a meditation on the anatomy of offense: how it arises, how it reshapes the offended soul, and how it can destroy ministries and beautiful relationships. Yet, this book is also a guide to healing.",
                "Through careful reflection, it teaches how to understand the signs of an offended heart, how to forgive, and how to restore the delicate fabric of trust and fellowship.",
                "This book is not just a leadership manual but also an exploration of human frailty and divine wisdom. It calls pastors, leaders, and believers alike to confront the inevitability of offense, and to embrace the higher law of forgiveness and reconciliation. In doing so, it offers a pathway to enduring relationships, resilient ministries, and the embodiment of Christ’s kingdom on earth.",
            ],
        },
    },
    {
        id: 9,
        title: "Great Salvation",
        slug: "great-salvation",
        image: bookImage9,
        description:
            "Eternity hangs in the balance. This book is an urgent summons to embrace the gospel of Christ, revealing the true path to salvation and equipping believers to live in the fullness of God’s grace.",
        price: "$20",
        pages: "160",
        year: "2025",
        category: "The Gospel & Salvation",
        availableAt: ["Amazon", "Church Bookstore", "Christian Bookshops"],
        formats: ["Hardcover", "Paperback", "E-Book"],
        sampleChapter: {
            title: "The Urgency of the Gospel",
            content: [
                "Eternity hangs in the balance, and there is a moment when a person’s eternal destiny is decided. Yet countless souls pursue pleasure and self-gratification, blind to the danger of eternal damnation. This book, Great Salvation, is a clarion call—an urgent summons to embrace the gospel of Christ before it is too late.",
                "It awakens readers to the origin and effects of sin, reveals the only true path to salvation through Christ, and equips believers to grow strong, walk in liberty, and live in the fullness of God’s grace.",
                "“How shall we escape if we neglect so great a salvation?” (Hebrews 2:3). This piercing question echoes through the ages, confronting every soul with the reality of eternity. Many seek relief from sin and fear of judgment through earthly gain, personal desire, and worldly distraction, yet none can deliver from the judgment of a Holy God.",
                "This book is a trumpet blast—a wake-up call to the urgency of the gospel. It challenges readers to examine their faith, reckon with eternal stakes, forsake their own ways, and embrace the life-transforming power of Christ.",
                "Earthly pleasures fade, but salvation in Christ endures forever. Whoever you are, Jesus can save you—He is the same yesterday, today, and forever, and He is still willing and ready to save all who come to Him.",
            ],
        },
    },
]
